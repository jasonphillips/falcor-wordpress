...
var TermsByIndices = (function (_FetchByIndices) {
  ...
  function TermsByIndices(shared, indices, filter, vocabulary, parentId) {
  ...
  _createClass(TermsByIndices, [{
    ...
    value: function getRootQuery() {
      var query = this.wp.taxonomies().collection(this.vocabulary);
      if (this.parentId) {
    ...
    value: function getReturnPath(index) {
      var base = ['taxonomies', this.vocabulary, this.parentId ? 'termsByParentId' : 'terms'];
      if (this.parentId) {
        base.push(this.parentId);
      }
      return typeof index !== 'undefined' ? base.concat([index]) : base;
    }
  }]);
